/**
 * Bucketing vectors shared with the equivalent implementations in the other Octopus OpenFeature
 * provider libraries (.NET, Java). The expected values are derived from the reference MurmurHash3
 * little-endian algorithm and are duplicated verbatim across all libraries. DO NOT modify the input
 * arguments or expected values — doing so would mask a real divergence in evaluation behaviour
 * between libraries.
 */
export const rolloutVectors: [string, string, number][] = [
